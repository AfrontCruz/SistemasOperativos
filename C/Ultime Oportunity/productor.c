...
void semWait(int sem, int n){
	...
	if( semop(sem, &sb, 1) != -1 ){
		//printf("Semaforo productor bloqueado %i\n",n);
	}
	else{
...
void semSignal(int sem, int n){
	struct sembuf sb = { n, 1, SEM_UNDO };
	if( semop(sem, &sb, 1) != -1 ){
		//printf("Semaforo consumidor abierto %i\n",n);
	}
	else
...
void escribirEnMem(int id_sem, char aux){
	int id = buscarSem(id_sem);
	printf("Productor.. Sem %i\n", id );
	semSignal(id_sem, id);
}
		...
		switch( proceso ){
			...
			case 0:
				...
				for( int x = 0; x < 1000; x++ ){
					semWait(id_sem, 0);
					escribirEnMem(id_sem, aux);
					semSignal(id_sem, 1);
				}
				
				printf("P%i.-Estoy terminando bien!\n", i);
				//sleep(5000);
